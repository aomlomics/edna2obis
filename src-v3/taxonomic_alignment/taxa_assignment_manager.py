"""
Taxonomic Alignment Manager for edna2obis
Contains ALL original notebook functionality for taxonomic alignment including:
...
def load_pr2_worms_dict_into_params(params, reporter=None, warn_print=None):
    ...
    into params['pr2_worms_dict']. Shared by assign_taxonomy and taxalign.
...
def assign_taxonomy(params, data, raw_data_tables, reporter):
    """
    Perform taxonomic alignment using WoRMS or GBIF API
    EXACT implementation from original notebook with ALL functionality preserved
    """
    try:
        reporter.add_section("Taxonomic Alignment")
        ...
        if matched_df is not None and not matched_df.empty:
            ...
            reporter.add_success(f"Taxonomic alignment completed! Saved {len(matched_df_final):,} records to occurrence_core_{api_source.lower()}.csv")
            ...
            reporter.add_error("Taxonomic alignment returned empty results")
            
    except Exception as e:
        reporter.add_error(f"Taxonomic alignment failed: {str(e)}")
...
def format_taxa_assignment_info_dataframe(taxa_info: pd.DataFrame, params: dict) -> pd.DataFrame:
    """
    Same column order, filled columns, and sorting as taxa_alignment_INFO.xlsx in the full workflow.
    Used by create_taxa_assignment_info and taxalign.py.
    """
...
def create_taxa_assignment_info(params, reporter):
    """
    Create a taxa_alignment_INFO.xlsx file (sheet taxa_alignment_INFO).
    For WoRMS and GBIF, this includes detailed candidate rows and an 'ambiguous' flag when available.
    """
    try:
        reporter.add_section("Creating Taxa Alignment Info File")
        ...
        output_filename = f"taxa_alignment_INFO_{api_choice}.xlsx"
        ...
        reporter.add_success("Taxa alignment info file created successfully")
        reporter.add_text(f"Saved taxa alignment info: {len(taxa_info):,} total rows ({taxa_info['verbatimIdentification'].nunique():,} unique strings)")
        ...
        reporter.add_text("<h3>Detailed Taxa Alignment Information</h3>")
        reporter.add_text(
            "<p>The table below (<code>taxa_alignment_INFO.xlsx</code>, sheet <code>taxa_alignment_INFO</code>) provides a comprehensive look at the results of the taxonomic matching process. "
            ...
            "<li><b>selected_match:</b> (WoRMS and GBIF) <strong>Primary column to use when reading this file:</strong> True on the one row per <code>verbatimIdentification</code> that was chosen for the final occurrence file (or, for taxalign-only runs, the row that <em>would</em> be chosen in the full pipeline). Other rows are alternate candidates for review.</li>"
        ...
        reporter.add_error(f"Taxa alignment info creation failed: {str(e)}")
